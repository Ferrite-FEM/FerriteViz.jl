...
# the refinement tolerances are a dataset property, shared by all its plots
ds = FEData(dh, u; adaptivity = Adaptivity(solution_tol = 2.0e-3, geometry_tol = 1.0e-3,
                                           max_depth = 6))

fig = WGLMakie.Figure(size = (800, 420))
ax = WGLMakie.Axis(fig[1, 1]; aspect = WGLMakie.DataAspect(), title = "solution")
solutionplot!(ax, ds; color = :u, colormap = :inferno, colorrange = (0.0, 1.0))
ax2 = WGLMakie.Axis(fig[1, 2]; aspect = WGLMakie.DataAspect(), title = "adapted mesh")
solutionplot!(ax2, ds; color = :u, colormap = :inferno, colorrange = (0.0, 1.0))
meshplot!(ax2, ds; color = (:white, 0.6), plotnodes = false)
